...
// gomobile-collection example/person.go
package society
...
func (v *PersonCollection) MarshalJSON() ([]byte, error) {
  return json.Marshal(v.s)
}

func (v *PersonCollection) UnmarshalJSON(data []byte) error {
...
func (v *PersonCollection) Insert(i int, n *Person) {
  if i < 0 || i > len(v.s) {
    fmt.Printf("Vapi::PersonCollection field_values.go error trying to insert at index %d\n", i)
    return
  }
  v.s = append(v.s, nil)
  copy(v.s[i+1:], v.s[i:])
...
func (v *PersonCollection) Remove(i int) {
  if i < 0 || i >= len(v.s) {
    fmt.Printf("Vapi::PersonCollection field_values.go error trying to remove bad index %d\n", i)
    return
  }
  copy(v.s[i:], v.s[i+1:])
  v.s[len(v.s)-1] = nil
...
func (v *PersonCollection) At(i int) *Person {
  if i < 0 || i >= len(v.s) {
    fmt.Printf("Vapi::PersonCollection field_values.go invalid index %d\n", i)
  }
  return v.s[i]
}
